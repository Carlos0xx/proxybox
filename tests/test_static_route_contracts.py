...
def test_service_restart_button_uses_css_pseudo_icon() -> None:
    ...
    # Compact cards (v0.2.x): min-height was dropped so button-less cards
    # render at their natural shorter height; restart button + icon shrank.
    assert "min-height: 182px" not in STATIC_HTML
    assert "background-size: 13px 13px" in STATIC_HTML
    assert "width: 13px" in STATIC_HTML
    assert "height: 30px" in STATIC_HTML
...
def test_services_view_renders_project_port_cards() -> None:
    ...
    # Services and ports render into two separate labelled sections.
    assert 'id="services-grid"' in STATIC_HTML
    assert 'id="ports-grid"' in STATIC_HTML
    assert 'id="ports-section-title"' in STATIC_HTML
    assert "系统服务" in STATIC_HTML
    assert "监听端口" in STATIC_HTML
    assert "repeat(auto-fit, minmax(240px, 1fr))" in STATIC_HTML
    # Uniform card height (no ragged tall/short): min-height on .svc-card +
    # default grid stretch, replacing the old variable-height layout.
    assert "min-height: 108px" in STATIC_HTML
    assert ".svc-grid" in STATIC_HTML
