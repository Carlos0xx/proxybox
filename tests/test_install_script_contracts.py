...
def test_unified_installer_prompts_for_docker_or_native() -> None:
    ...
    # Enter defaults to Docker so Linux novices can one-click through the prompt.
    assert "直接回车 = 选择 1(Docker，推荐)。" in INSTALL_SH
    assert "请选择 [1/2]（回车默认 1）: " in INSTALL_SH
    assert '""|1|d|D|docker|Docker|DOCKER)' in INSTALL_SH
