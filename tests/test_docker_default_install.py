...
def test_install_red_line_is_documented_for_docker_path() -> None:
    zh_red_line = "安装红线: 不要删除、修改、覆盖或复用用户 VPS 上本次安装以外的任何文件和服务"
    zh_scope = "即便宿主机已经存在 `/opt/proxybox` 或同名目录,也必须保留不动"
    en_red_line = "Installation red line: never delete, modify, overwrite, or reuse files"
    en_scope = "`/opt/proxybox` or another same-name directory already exists"

    assert zh_red_line in README_ZH
    assert zh_scope in README_ZH
    assert en_red_line in DOCKER_DOC
    assert en_scope in DOCKER_DOC
...
def test_bootstrap_uses_docker_env_ports(monkeypatch, tmp_path: Path) -> None:
    ...
    # SNI lands in the sing-box config and is one of the validated pool.
    sni = singbox_cfg["inbounds"][0]["tls"]["server_name"]
    assert sni in bootstrap.SNI_CANDIDATES
    assert singbox_cfg["inbounds"][1]["masquerade"] == f"https://{sni}"

    creds = bootstrap._gen_proxybox_config(pb_dir, cover_domain=sni)
    ...
    assert proxybox_cfg["server"]["cover_domain"] == sni
