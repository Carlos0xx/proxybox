...
If the user has not explicitly answered "Docker" / "1" or "native" / "2",
stop before Step 1 and ask:

> Choose install mode: 1) Docker install (recommended, isolated containers,
> safer for VPS hosts that already run other services) 2) native install
> (advanced, writes host Python/systemd/fail2ban, only for clean dedicated VPSes)
...
SSH host-key material from this template flow.
...
> "Installed. The handoff below has the login credentials, first device,
> service status, and optional next steps. Open the login URL in a browser and
> save the password before closing the terminal."
...
1. **Open the login URL in a browser**. The form auto-focuses the username
   field, then type the username + password from the summary and submit,
...
- **HTTPS:** Docker installs can use the panel's HTTPS page. The container
  ...
  hand.
- **Change username / password (v0.1.11):** Security page → login settings
  card. Password change requires the current password (session-hijack
  defense).
- **Rotate login path (v0.1.11):** same card → rotate. Old `/login/{x}`
  immediately 404s; existing sessions unaffected. Defends against
  `/login` brute-force.
- **Add more devices:** Devices page → create. Generic naming convention:
  `phone-2`, `tablet-1`, `laptop-1`, `home-router`, or lowercase random strings. **Never
  use personal identifiers** — device names are written into sing-box config
  and subscription content.
- **Per-device subscription URLs:** Subscription page or
  Devices page → device → subscription URL. The copy button works on plain
  HTTP too (v0.1.12 patched the textarea fallback that newer browsers
  refused). If a user reports that the copy button does nothing, they're on a SPA
...
- **Live throughput + per-device history:** Overview, device history, and total traffic.
...
- **Never** SSH with `-o StrictHostKeyChecking=no` without the user's
  explicit consent — it disables host-key warnings and could hide MITM
