...
if [ "$LANG_CHOICE" = "zh" ]; then
    ...
    M_SECTION_PORTS_TITLE="🔓 防火墙放行端口 (本次随机分配)"
    M_SECTION_PORTS_HINT="在云厂商安全组 / 主机防火墙放行下面这两段,客户端才连得上"
    M_PORTS_VLESS_LABEL="VLESS (TCP)"
    M_PORTS_HY2_LABEL="Hy2   (UDP)"
    ...
    M_SINGBOX_VERSION="    sing-box: %s"
    M_GEN_KEYPAIR="==> generating protocol keys + Hy2 certificate..."
    ...
    M_SECTION_PORTS_TITLE="🔓 firewall ports (randomised for this install)"
    M_SECTION_PORTS_HINT="open these two ranges in your cloud security group / host firewall, or clients can't connect"
    M_PORTS_VLESS_LABEL="VLESS (TCP)"
    M_PORTS_HY2_LABEL="Hy2   (UDP)"
...
if [ ! -f "$SINGBOX_DIR/config.json" ]; then
    ...
    # Reality / Hy2 cover domain. We avoid the canonical
    # apple/microsoft/cloudflare/amazon set (those are the oldest Reality
    # examples and are themselves a fingerprint). This is a wider pool of
    # high-traffic, non-canonical sites. PROXYBOX_SNI overrides it — set
    # your own (validate with scripts/check-sni.py) for the best result;
    # a hard-coded pool in open-source code is itself a weaker fingerprint.
    # Each verified to negotiate TLS 1.3 + HTTP/2 (h2). Big-brand marketing
    # sites behind WAFs (nvidia/intel/samsung/...) only offer HTTP/1.1 and
    # make poor cover domains, so this pool leans dev/tech/business.
    SNI_CANDIDATES=(
        www.python.org www.djangoproject.com www.ruby-lang.org nodejs.org
        www.kernel.org www.debian.org www.docker.com www.npmjs.com
        www.heroku.com www.cloudera.com www.udemy.com www.squarespace.com
        www.fastly.com www.zoom.us
    )
    if [ -n "${PROXYBOX_SNI:-}" ]; then
        SNI="$PROXYBOX_SNI"
    else
        SNI="${SNI_CANDIDATES[$RANDOM % ${#SNI_CANDIDATES[@]}]}"
    fi
    ...
    # Randomise the port base so deployments don't all sit on 11000/21000
    # (a fixed port is a fingerprint). VLESS template = VLESS_BASE, devices
    # take VLESS_BASE+1..+50; Hy2 likewise. Ranges are kept non-overlapping
    # (VLESS in 10000-28999, Hy2 in 31000-54999). PROXYBOX_*_BASE overrides
    # for reproducible / scripted installs.
    VLESS_BASE="${PROXYBOX_VLESS_BASE:-$(( 10000 + (RANDOM % 19000) ))}"
    HY2_BASE="${PROXYBOX_HY2_BASE:-$(( 31000 + (RANDOM % 24000) ))}"
    VLESS_RANGE_START=$(( VLESS_BASE + 1 )); VLESS_RANGE_END=$(( VLESS_BASE + 50 ))
    HY2_RANGE_START=$(( HY2_BASE + 1 ));     HY2_RANGE_END=$(( HY2_BASE + 50 ))
...
if [ ! -f "$CONFIG_DIR/config.yaml" ]; then
    ...
    PUBLIC_HOST=$(curl -4 -fsS --max-time 5 https://api4.ipify.org 2>/dev/null \
                 ...
                 || echo "")
    # $SNI / port bases are set in the sing-box block above on a fresh
    # install. If that block was skipped (config.json already present),
    # recover the values from the existing sing-box config so config.yaml
    # stays consistent with the running inbounds.
    if [ -z "${SNI:-}" ] && [ -f "$SINGBOX_DIR/config.json" ]; then
        SNI=$(.venv/bin/python -c "import json,sys; c=json.load(open('$SINGBOX_DIR/config.json')); print(next((i.get('tls',{}).get('server_name','') for i in c.get('inbounds',[]) if i.get('tls',{}).get('server_name')), ''))" 2>/dev/null || echo "")
    fi
    if [ -z "${VLESS_RANGE_START:-}" ] && [ -f "$SINGBOX_DIR/config.json" ]; then
        VLESS_BASE=$(.venv/bin/python -c "import json; c=json.load(open('$SINGBOX_DIR/config.json')); print(next((i['listen_port'] for i in c.get('inbounds',[]) if i.get('type')=='vless'), 11000))" 2>/dev/null || echo 11000)
        HY2_BASE=$(.venv/bin/python -c "import json; c=json.load(open('$SINGBOX_DIR/config.json')); print(next((i['listen_port'] for i in c.get('inbounds',[]) if i.get('type')=='hysteria2'), 21000))" 2>/dev/null || echo 21000)
        VLESS_RANGE_START=$(( VLESS_BASE + 1 )); VLESS_RANGE_END=$(( VLESS_BASE + 50 ))
        HY2_RANGE_START=$(( HY2_BASE + 1 ));     HY2_RANGE_END=$(( HY2_BASE + 50 ))
    fi
...
services:
  monitored:
    ...
    - fail2ban
ports:
  vless_range: [$VLESS_RANGE_START, $VLESS_RANGE_END]
  hy2_range: [$HY2_RANGE_START, $HY2_RANGE_END]
clash:
...
# Read the (randomised) port ranges back from config.yaml so the summary
# reflects the running state even on a re-run where they weren't generated
# this pass.
PORTS_VLESS=$(.venv/bin/python -c "import yaml; r=yaml.safe_load(open('$CONFIG_DIR/config.yaml'))['ports']['vless_range']; print(f'{r[0]}-{r[1]}')" 2>/dev/null || echo "?")
PORTS_HY2=$(.venv/bin/python -c "import yaml; r=yaml.safe_load(open('$CONFIG_DIR/config.yaml'))['ports']['hy2_range']; print(f'{r[0]}-{r[1]}')" 2>/dev/null || echo "?")
...
# ── Firewall ports block — ports are randomised, tell the user ──────
printf "  %s%s%s  %s— %s%s\n" "$C_CYAN_B" "$M_SECTION_PORTS_TITLE" "$C_RESET" "$C_DIM" "$M_SECTION_PORTS_HINT" "$C_RESET"
echo ""
printf "      %s%-12s%s %s%s/tcp%s\n" "$C_DIM" "$M_PORTS_VLESS_LABEL" "$C_RESET" "$C_GREEN_B" "$PORTS_VLESS" "$C_RESET"
printf "      %s%-12s%s %s%s/udp%s\n" "$C_DIM" "$M_PORTS_HY2_LABEL" "$C_RESET" "$C_GREEN_B" "$PORTS_HY2" "$C_RESET"
echo ""
