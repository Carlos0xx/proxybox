...
# VLESS / Hy2 ports are randomised per install (picked after this pre-flight
# runs), so there are no fixed proxy ports to probe here. install.sh prints
# the chosen ranges in its summary; sing-box surfaces a bind error if a
# random pick ever collides with an existing service.
echo "  $M_PORTS_NOTE"
