...
Then in any session: *"deploy proxybox on my VPS at 1.2.3.4 using ~/.ssh/id_ed25519"*. The agent uses a temporary SSH `known_hosts` → runs a minimal VPS check → `git clone` / update → full pre-flight with Python 3.11 provisioning → `install.sh` → service verification → hands back the login URL + credentials.
