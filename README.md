...
**English** · [Chinese README](./README.zh.md)
...
| 🌐 &nbsp; **VLESS Reality + Hysteria2** | Dual-protocol per-device access with separate TCP and UDP paths. |
...
Let an AI coding agent drive the install over SSH — the lowest-friction path if you already use Claude Code or Codex. For Claude Code, install the bundled skill once:
...
### B · One-line install *(just a VPS, no local tools)*
...
It installs git/curl, clones ProxyBox to `/opt/proxybox`, and launches the installer. You only have to answer one question — Docker or native install — and **pressing Enter picks Docker**, the recommended mode. To skip even that prompt, append a flag:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/carlos0xx/proxybox/main/deploy/get.sh) --docker
bash <(curl -fsSL https://raw.githubusercontent.com/carlos0xx/proxybox/main/deploy/get.sh) --native --fresh --lang en
```

Prefer to read the script before running it? Download first: `curl -fsSL https://raw.githubusercontent.com/carlos0xx/proxybox/main/deploy/get.sh -o get.sh`, then `less get.sh && bash get.sh`.

Pick **Docker** for container isolation, automatic port selection, an install-scoped Docker guard, and an HTTPS helper that only runs when you enable HTTPS from the panel. If the VPS already runs websites, panels, or production services, use Docker. Native install writes Python, sing-box, systemd units, and fail2ban directly to the host; only use it on a clean dedicated VPS.

Already have the repo cloned, or want to control the install directory yourself? Run the installer directly:

```bash
cd <proxybox-checkout> && bash deploy/install.sh           # interactive mode picker
cd <proxybox-checkout> && bash deploy/install.sh --docker  # or --native --fresh --lang en
```
...
├── static/     Web UI — single-file SPA served by the backend
