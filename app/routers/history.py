...
Per-host and per-app-group rollups are served inside
/history/device/{name} (from the host_log table), not as a separate
endpoint.
"""
...
async def history_device(name: NameInPath, days: DaysQuery = 7) -> dict:
    """Single device — hourly buckets, daily rollup, device metadata,
    plus per-host and per-app-group rollups from host_log (v0.1.9+).

    The SPA's `renderHistoryDevice` reads `d.device.label`, `d.daily`,
    `d.hosts`, `d.apps`. Host/app lists are empty until the traffic worker
    has populated host_log; the SPA renders that as an empty state.
    """
    cutoff = _cutoff_ts(days)
    with connection() as conn:
        rows = conn.execute(
