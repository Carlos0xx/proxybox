...
first match by suffix. Unknown hosts fall to ``UNKNOWN_GROUP``.
"""

from __future__ import annotations

# Catch-all group for hosts not in the table (IP literals, unknown domains).
# The value is part of the app-group vocabulary shown in the SPA, so it is
# kept as data rather than translated.
UNKNOWN_GROUP = "其他"
...
def classify(host: str | None) -> str:
    """Return the app group for a host name, or UNKNOWN_GROUP if unknown.
    ...
    suffix that's in the lookup table. Examples:
        ...
        chat.openai.com              → 'AI'
        unknown.example.org          → UNKNOWN_GROUP
        ""                           → UNKNOWN_GROUP
        IPv4 address                 → UNKNOWN_GROUP
    """
    if not host:
