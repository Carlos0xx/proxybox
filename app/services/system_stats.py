...
def cpu_pct() -> str:
    """Whole-host CPU utilisation %, computed from /proc/stat.

    Reads /proc/stat directly (two short samples, busy = 1 - idle/total) so
    it needs no procps/``top`` — the Docker image is slim and ships no top,
    which made the old top-based reading always report 0. Falls back to
    parsing ``top -bn1`` only if /proc/stat is unreadable.
    """
    ...
    if first is not None:
        ...
        if second is not None:
            total_d = second[0] - first[0]
            idle_d = second[1] - first[1]
            if total_d > 0:
                pct = (1.0 - idle_d / total_d) * 100.0
                return f"{max(0.0, min(100.0, pct)):.1f}"

    # Fallback for hosts where /proc/stat is unavailable.
    for line in run(["top", "-bn1"]).splitlines():
        s = line.lstrip()
        if not s.startswith("%Cpu"):
            continue
        # %Cpu(s):  1.2 us,  0.4 sy, ...  (locale may use ',' for '.')
        parts = s.replace(",", ".").split()
        try:
            return f"{float(parts[1]) + float(parts[3]):.1f}"
