...
`GET /api/sub/<sub_token>` returns plain text, one client URI per line:
VLESS first, then Hysteria2. The exact query parameters are generated from the
current sing-box template and are intentionally treated as an implementation
detail.
