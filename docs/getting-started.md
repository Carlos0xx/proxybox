...
Let an AI coding agent drive the install over SSH — the lowest-friction option if you already have Claude Code or Codex open.
...
## Path 2 — One-line install &nbsp;<sub>(just a VPS, no local tools)</sub>
...
It installs git/curl, clones ProxyBox to `/opt/proxybox`, and launches the installer. The only question is Docker or native install — **pressing Enter picks Docker**, the recommended mode. Append a flag to skip even that:
...
To read the script before running it: `curl -fsSL https://raw.githubusercontent.com/carlos0xx/proxybox/main/deploy/get.sh -o get.sh`, then `less get.sh && bash get.sh`.

Pick Docker to check Docker/Compose and `ss`/`iproute2`, install missing runtime packages, start Docker, scan host ports, write `.env`, start an isolated bridge-network stack, and install narrow Docker guard + HTTPS helper units for this project only. Use Docker if the VPS already runs websites, panels, or production services. Native install writes Python, sing-box, systemd units, and fail2ban directly to the host; only use it on a clean dedicated VPS.

Already have the repo cloned, or want to choose the install directory yourself? Run the installer directly:
...
4. **Protocol material generation** — protocol keys and the Hy2 self-signed certificate are generated per install.
